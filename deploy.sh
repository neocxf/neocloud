...
#  mvn --projects $1 clean package
  ...
  export WORKDIR=/home/neo/pub
  scp $1/target/$1.jar neo@192.168.2.104:$WORKDIR
  ssh neo@192.168.2.104 << ENDSSH

    cd $WORKDIR

    mkdir -p bash/ jars/ logs/ pids/

    mv $1.jar jars/

    echo "PID='ps -ef | grep java | grep $1'" > $1.sh

    echo "nohup java $JAVA_OPTS -jar  -DHOST=$HOST -DZK_HOST=$ZK_HOST -DREDIS_HOST=$REDIS_HOST -DMY_HOST=$MY_HOST -DMQ_HOST=$MQ_HOST $WORKDIR/jars/$1.jar >> $WORKDIR/logs/$1.log &" >> $1.sh

    chmod u+x $1.sh

    mv $1.sh bash/

    sh $WORKDIR/bash/$1.sh
